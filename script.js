...
function generateResponse(userInput) {
    ...
    // Überprüft auf mathematische Berechnungen
    for (const [key, response] of Object.entries(responsesDe)) {
        const regex = new RegExp(key.replace(/(.*)/g, "($1)"), 'i');
        const match = userInput.match(regex);
        if (match) {
            return typeof response === 'function' ? response(match, ...match.slice(1)) : response;
        }
    }
